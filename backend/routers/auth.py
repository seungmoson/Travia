...
#  LoginResponse 외 SignupRequest, UserPublic 스키마 import
...
def create_user(request: SignupRequest, db: Session = Depends(get_db)):
    ...
    # 3. 새 User 객체 생성
    #     nickname 필드에 request.username 값을 할당
    new_user = User(
        email=request.email,
...
#  tokenUrl은 /auth/login (라우터 prefix 포함)
